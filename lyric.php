...
class Ac_KGenius
{
	...
	private function parseSearchResult ( $content, $artist, $title )
	{
		...
		foreach ( $dom->getElementsByTagName ( 'li' ) as $li )
		{
			...
			//Check Artist
			if ( $this->StringContains ( $genius_artist, $artist ) )
			{		
				...
				//Check Title
				if ( $this->StringContains ( $genius_title, $title ) )
				{
	...
	public function getLyrics ( $id, $info )
	{
		...
		$begin = '<lyrics class="lyrics" remove-class-on-angular-load="lyrics" yields-anchorer="lyrics_anchorer = anchorer" canonical-lyrics-html="lyrics_data.body.html" radio-edit>';
		$end   = '</lyrics>';
	...
	private function StringContains ( $substring, $string ) 
	{
